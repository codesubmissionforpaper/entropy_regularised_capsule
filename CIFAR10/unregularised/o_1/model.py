...
class ResnetCnnsovnetDynamicRouting(nn.Module):
    def __init__(self):
        ...
        self.conv_caps1 = ConvCapsule(in_caps=16,in_dim=16,out_caps=16,out_dim=16,kernel_size=5,stride=1,padding=0)
        self.conv_caps2 = ConvCapsule(in_caps=16,in_dim=16,out_caps=16,out_dim=16,kernel_size=3,stride=2,padding=0)
        self.conv_caps3 = ConvCapsule(in_caps=16,in_dim=16,out_caps=16,out_dim=16,kernel_size=3,stride=1,padding=0)
        self.conv_caps4 = ConvCapsule(in_caps=16,in_dim=16,out_caps=16,out_dim=16,kernel_size=3,stride=1,padding=1)        
        self.class_caps = ConvCapsule(in_caps=16,in_dim=16,out_caps=10,out_dim=16,kernel_size=3,stride=1,padding=0)
    ...
    def forward(self,x):
        ...
        entropy += temp.mean()
        #print(entropy.size())
        class_caps, temp = self.class_caps(conv_caps4)
        entropy += temp.mean()
        #print(entropy.size())
