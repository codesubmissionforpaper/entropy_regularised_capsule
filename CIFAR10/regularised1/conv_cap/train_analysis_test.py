...
def train(epoch,trainloader,trial):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        weight = max((-0.8/199)*(epoch+1)+(1+(0.8/199)),0.2)
        criterion = weight*loss_criterion(outputs, targets) + (1-weight)*entropy_sum
        loss = criterion
        loss.backward()
        optimizer.step()
        first_term += float(criterion)
        e += float(entropy_sum.sum())           
        ...
        if batch_idx%100 == 0:
           ...
           print(first_term/(batch_idx+1),e/(batch_idx+1))
    scheduler.step()
    with torch.no_grad():
         ...
         state = {
                  ...
                  'first_term': first_term/(batch_idx+1),
                  'entropy_loss_sum': e/(batch_idx+1),
                  'acc': correct/total,
                  'epoch': epoch
                  }
...
def test(epoch,testloader,trial):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(testloader):
            ...
            e += float(entropy_sum.sum())
            weight = max((-0.8/199)*(epoch+1)+(1+(0.8/199)),0.2)         
            test_loss += weight*float(criterion) + (1-weight)*float(entropy_sum.sum())
            _, predicted = outputs.max(1)
            total += targets.size(0)
            correct += predicted.eq(targets).sum().item()
