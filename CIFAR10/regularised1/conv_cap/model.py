...
'''class Model(nn.Module):
      ...
      def __init__(self):
          ...
          self.capsule_layer_one = ConvSov(16,16,3,8,8,2)
          #(batch_size,16,7,7,16)
          self.capsule_layer_two = ConvSov(16,16,2,8,16,2)
...
class Model(nn.Module):
      ...
      def __init__(self):
          super(Model,self).__init__()
          #(batch_size,1,32,32) -> (batch_size,64,31,31) 
          self.conv = nn.Sequential(nn.Conv2d(3,32,3,1,1),
                                    nn.ReLU()
                       )
          #(batch_size,32,16,31,31) 
          self.primary_capsule =  PrimaryCapsules(32,16,8)
          #(batch_size,16,15,15,16)
          self.capsule_layer_one = ResidualBlock(16,16,8,8)
          #(batch_size,16,7,7,16)
          self.capsule_layer_two = ResidualBlock(16,16,8,16)
          #(batch_size,16,3,3,16)
          self.capsule_layer_three = ResidualBlock(16,16,16,16)
          #(batch_size,5,1,1,16)
          self.class_capsule = ConvSov(16,10,3,16,16,1)
          #(batch_size,in_channel,im_size,im_size)
          self.linear = nn.Linear(16,1)            
