...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.conv1 = P4MConvP4M(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False) # stride 1: 64 -> 64 | stride 2: 64 -> 32
        self.bn1 = nn.BatchNorm3d(planes)
        self.conv2 = P4MConvP4M(planes, planes, kernel_size=3, stride=1, padding=1, bias=False) # 64 -> 64 ie same
...
class ResNetPreCapsule(nn.Module):
    def __init__(self, block, num_blocks):
        ...
        self.conv1 = P4MConvZ2(1, 16, kernel_size=3, stride=1, padding=1, bias=False)#(b_size,16,32,32)
...
class PrimaryCapsules(nn.Module):
    def __init__(self,in_channels,num_capsules,out_dim,H=16,W=16):
        ...
        self.H = H
        self.W = W
        self.preds = nn.Sequential(P4MConvP4M(in_channels,num_capsules*out_dim,kernel_size=1),
                                   nn.SELU(),
                                   nn.LayerNorm((num_capsules*out_dim,8,H,W)))

    def forward(self,x):
        # x : (b,64,8,16,16)
        primary_capsules = self.preds(x) #(b,16*8,8,16,16)
        primary_capsules = primary_capsules.view(-1,self.num_capsules,self.out_dim,8,self.H,self.W)
        return primary_capsules #(b,16,8,8,16,16)
...
class ConvCapsule(nn.Module):
    def __init__(self,in_caps,in_dim,out_caps,out_dim,kernel_size,stride,padding):
        ...
        self.out_dim = out_dim
        self.preds = nn.Sequential(P4MConvP4M(in_dim,out_caps*out_dim,kernel_size=kernel_size,stride=stride,padding=padding),
                                   nn.BatchNorm3d(out_caps*out_dim),
                                   nn.SELU())
     
    def forward(self,in_capsules,ITER=3):
        # in_capsules : (b,16,8,8,16,16)
        batch_size, _, _, _, H, W = in_capsules.size()
        in_capsules = in_capsules.view(batch_size*self.in_caps,self.in_dim,8,H,W) #(b*16,8,8,16,16)
        predictions = self.preds(in_capsules) # (b,)
        _,_,_, H, W = predictions.size()
        predictions = predictions.view(batch_size, self.in_caps, self.out_caps*self.out_dim, 8, H, W)
        predictions = predictions.view(batch_size, self.in_caps, self.out_caps, self.out_dim, 8, H, W)
    ...
    def unif_act_wt_entropy(self, c_ij):
        ...
        return (-1/(N*I*H*W*8)) * torch.sum(torch.sum(c_ij * (torch.log10(c_ij + EPS)/0.69897000433), dim=2))    
    ...
    def dynamic_routing(self,predictions,ITER=3):
        ...
        b_ij = torch.zeros(batch_size,self.in_caps,self.out_caps,1,8,H,W).to(DEVICE)
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.class_caps = ConvCapsule(in_caps=16,in_dim=16,out_caps=5,out_dim=16,kernel_size=5,stride=2,padding=0) # (1,1)
