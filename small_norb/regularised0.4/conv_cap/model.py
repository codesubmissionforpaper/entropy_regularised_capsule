...
class Model(nn.Module):
      ...
      def __init__(self):
          ...
          #(batch_size,1,36,36) -> (batch_size,128,18,18) 
          self.conv = PreActResNet(PreActBottleneck,[3,4])
          #(batch_size,32,18,18,8) 
          self.primary_capsule =  PrimaryCapsules(512,32,8)
          #(batch_size,32,8,8,8)
          self.capsule_one = ConvSov(32,32,3,8,8,2)
          #(batch_size,16,6,6,16)
          self.capsule_two = ConvSov(32,16,3,8,16,1)
          #(batch_size,16,4,4,16)
          self.capsule_three = ConvSov(16,16,3,16,16,1)
          #(batch_size,5,1,1,16)
      ...
      def forward(self,input):
          ...
          capsule_one, capsule_one_c_ij = self.capsule_one(primary_capsule)
          capsule_two, capsule_two_c_ij = self.capsule_two(capsule_one)
          capsule_three, capsule_three_c_ij = self.capsule_three(capsule_two)
          class_capsule, class_capsule_c_ij = self.class_capsule(capsule_three)
          class_capsule = class_capsule.squeeze(2).squeeze(2)
          class_predictions = self.linear(class_capsule).squeeze(2)
          return class_predictions, capsule_one_c_ij+capsule_two_c_ij+capsule_three_c_ij+class_capsule_c_ij
