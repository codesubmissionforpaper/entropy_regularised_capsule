...
class ConvCapsule(nn.Module):
    ...
    def forward(self,in_capsules,ITER=3):
        ...
        predictions = predictions.view(batch_size, self.in_caps, self.out_caps, self.out_dim, H, W)
        _, in_dim, H, W = in_capsules.size()
        in_capsules = in_capsules.view(batch_size,self.in_caps,in_dim,H,W)
        in_capsule_norm = torch.norm(in_capsules,dim=2)
        in_capsule_norm = F.max_pool2d(in_capsule_norm,kernel_size = (self.kernel_size,self.kernel_size), stride = (self.stride,self.stride), padding = (self.padding,self.padding))
