...
class ConvCapsule(nn.Module):
    ...
    def dynamic_routing(self,predictions,ITER=3):
        ...
        predictions_permute = predictions.permute(0,2,4,5,1,3)#(batch_size,num_out_capsules,H,W,num_in_capsules,out_capsule_dim)
        affinity_matrices = self.cosine_similarity(predictions_permute)    
        degree_score = torch.sum(affinity_matrices,dim=-1,keepdim=True)#(batch_size,num_out_capsules,H,W,num_in_capsules,1)
        degree_score = (degree_score).permute(0,4,1,5,2,3)#(batch_size,num_in_capsules,num_out_capsules,1,4,H,W)
        predictions = degree_score*predictions
...
class ResnetCnnsovnetDynamicRouting(nn.Module):
    def __init__(self):
        ...
        self.primary_caps = PrimaryCapsules(32,8,16,30,30)#for cifar10, H,W = 16, 16. For MNIST etc. H,W = 14,14.
        self.conv_caps1 = ConvCapsule(in_caps=8,in_dim=16,out_caps=8,out_dim=16,kernel_size=3,stride=1,padding=0) # (7,7)
        self.conv_caps2 = ConvCapsule(in_caps=8,in_dim=16,out_caps=16,out_dim=16,kernel_size=3,stride=2,padding=0) # (5,5)
        self.conv_caps3 = ConvCapsule(in_caps=16,in_dim=16,out_caps=16,out_dim=16,kernel_size=5,stride=1,padding=0) # (1,1)
        self.conv_caps4 = ConvCapsule(in_caps=16,in_dim=16,out_caps=4,out_dim=16,kernel_size=3,stride=2,padding=0) # (1,1)
        self.class_caps = ConvCapsule(in_caps=4,in_dim=16,out_caps=2,out_dim=16,kernel_size=4,stride=1,padding=0) # (1,1)
